...
struct VaccineRecommendations {
    ...
    static func recommendedReminders(for profile: Profile) -> [Reminder] {
        ...
        if age >= 50 {
            reminders.append(makeReminder(title: "Shingles Vaccine (2 doses starting at 50)", type: "Vaccine", yearsFromNow: 0))
        }
        
        if age >= 65 {
            reminders.append(makeReminder(title: "Pneumococcal Vaccine", type: "Vaccine", yearsFromNow: 0))
        }
        
        if gender == "Female" && age >= 21 {
            reminders.append(makeReminder(title: "Cervical Cancer Screening (Pap Smear every 3 years)", type: "Screening", yearsFromNow: 0))
        }
        
        if gender == "Female" && age >= 40 {
            reminders.append(makeReminder(title: "Mammogram (every 2 years)", type: "Screening", yearsFromNow: 0))
        }
        ...
        if age >= 20 {
            reminders.append(makeReminder(title: "Cholesterol Screening (every 5 years)", type: "Screening", yearsFromNow: 0))
        }
        
        if age >= 18 {
            reminders.append(makeReminder(title: "Blood Pressure Check (every 2 years)", type: "Screening", yearsFromNow: 0))
        }
        
        if gender == "Female", age >= 65 {
            reminders.append(makeReminder(title: "Osteoporosis Screening", type: "Screening", yearsFromNow: 0))
        }
